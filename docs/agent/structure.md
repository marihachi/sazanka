...
- `engine/` … 回路のデータと計算処理。React や DOM に依存させない。データ構造と部品の種類ごとの仕様（`project.ts`）、部品の評価（`sim.ts`）、モジュールの展開（`flatten.ts`）、回路の編集（`edit.ts`）、部品の大きさと配置（`layout.ts`）、共有用 JSON（`share.ts`）が入る。
...
テストは、React に依存しない処理について、対象のファイルと同じフォルダに置く（`engine/*.test.ts`、`app/*.test.ts`）。
