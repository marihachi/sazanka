...
- Claude Code では、Edit / Write でファイルを編集するたびに、フック（`.claude/settings.json` → `.claude/hooks/format.mjs`）が Prettier で整形する。ほかのエージェントやシェルからの書き込み（sed、スクリプトなど）には効かないので、その場合は自分で整形する。
